...
| `CLAWORC_CF_ACCESS_ENABLED` | `false` | Enable [Cloudflare Access](/authentication#cloudflare-access-zero-trust) header authentication. Replaces the built-in login. |
| `CLAWORC_CF_ACCESS_TEAM_DOMAIN` | _(empty)_ | Cloudflare Access team domain, e.g. `https://myteam.cloudflareaccess.com`. Required when Cloudflare Access is enabled. |
| `CLAWORC_CF_ACCESS_AUD` | _(empty)_ | Cloudflare Access application AUD tag. Required when Cloudflare Access is enabled. |
